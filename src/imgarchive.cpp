...
void IMGArchive::ExportAll(ArchiveInfo *pInfo) {
  ...
  for (size_t i = 0; i < total; ++i) {
    std::string path =
        std::format("{}\\{}", pInfo->path, pInfo->pArc->EntryList[i].FileName);
    pInfo->pArc->ExportEntry(&pInfo->pArc->EntryList[i], path, false);
    pInfo->pArc->ProgressBar.Percentage =
        (static_cast<float>(i) + 1) / static_cast<float>(total);
...
void IMGArchive::ExportSelected(ArchiveInfo *pInfo) {
  ...
  for (size_t i = 0; i < total; ++i) {
    if (pInfo->pArc->EntryList[i].bSelected) {
      std::string path = std::format("{}\\{}", pInfo->path,
                                     pInfo->pArc->EntryList[i].FileName);
      pInfo->pArc->ExportEntry(&pInfo->pArc->EntryList[i], path, false);
      pInfo->pArc->ProgressBar.Percentage =
          (static_cast<float>(i) + 1) / static_cast<float>(total);
