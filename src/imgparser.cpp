...
void Parser::Open(IMGArchive *pArc) {
  ...
  if (std::filesystem::exists(dirPath)) {
    pArc->FileName =
        std::move(std::filesystem::path(dirPath).filename().stem().string());
...
void Parser::Export(IMGArchive *pMgr, EntryInfo *pEntry,
                    const std::string &filePath, bool logMsg) {
...
void Parser::Import(IMGArchive *pArc, const std::string &path, bool replace) {
  ...
  if (replace) {
    pArc->EntryList.erase(std::remove_if(pArc->EntryList.begin(),
                                         pArc->EntryList.end(),
                                         [&](EntryInfo const &obj) {
  ...
  info.Size = static_cast<uint32_t>(std::filesystem::file_size(path)) /
              2048; // bytes -> sector
...
void Parser::Save(ArchiveInfo *pInfo) {
  ...
  if (fImg) {
    ...
    for (EntryInfo &e : pInfo->pArc->EntryList) {
      ...
      pInfo->pArc->ProgressBar.Percentage =
          (static_cast<float>(index) + 1) / static_cast<float>(total);
  ...
  pInfo->pArc->FileName =
      std::move(std::filesystem::path(pInfo->path).filename().stem().string());
  pInfo->pArc->ProgressBar.bInUse = false;
