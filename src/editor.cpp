...
void Editor::AboutPopUp() {
  ...
  if (ImGui::Button("GitHub", Widget::CalcSize(2))) {
    ShellExecute(nullptr, "open", "https://github.com/user-grinch/IMGEditor/",
                 nullptr, nullptr, SW_SHOWNORMAL);
  }
  ImGui::SameLine();
  if (ImGui::Button("Patreon", Widget::CalcSize(2))) {
    ShellExecute(nullptr, "open", "https://www.patreon.com/grinch_", nullptr,
...
void Editor::UpdatePopUp() {
  ...
  Widget::TextCentered(
      std::format("Latest version: {}", Updater::GetUpdateVersion()));
  ImGui::Spacing();
  Widget::TextCentered("Update to continue using the Editor");
  ImGui::Spacing();
  ImGui::TextWrapped("Newer versions may contain more features & bug fixes. "
                     "Updates will be optional once the editor is stable.");
  ImGui::Dummy(ImVec2(0, 15));
  if (ImGui::Button("Download update", Widget::CalcSize())) {
    ShellExecute(nullptr, "open",
                 "https://github.com/user-grinch/IMGEditor/releases/", nullptr,
                 nullptr, SW_SHOWNORMAL);
  }
  ImGui::Spacing();
  ImGui::Text("Copyright Grinch_ 2022-2023. All rights reserved.");
...
void Editor::WelcomePopup() {
  ...
  ImGui::Spacing();
  ImGui::TextWrapped("This editor is still work in progress. There may be LOT "
                     "of bugs. Feel free to report if you find any.");
  ...
  ImGui::Spacing();
  ImGui::TextWrapped("Directly open archives in IMG Editor by setting windows "
                     "file association.");
  ImGui::Spacing();
  if (ImGui::Button("Help with file association", Widget::CalcSize(1))) {
    ShellExecute(
        nullptr, "open",
        "https://www.google.com/search?q=windows+set+file+associations",
        nullptr, nullptr, SW_SHOWNORMAL);
  }
  ImGui::Spacing();
  if (ImGui::Button("GitHub", Widget::CalcSize(2))) {
...
const char *Editor::GetFilterText() { return Filter.InputBuf; }
...
void Editor::ProcessMenuBar() {
  if (ImGui::BeginMenu("File")) {
    ...
    if (ImGui::MenuItem("Open...")) {
      ...
      std::string fileName =
          std::filesystem::path(path).filename().stem().string();
    ...
    if (ImGui::MenuItem("Save", NULL, false,
                        pSelectedArchive && !pSelectedArchive->Path.empty())) {
      if (!pSelectedArchive->ProgressBar.bInUse) {
        ArchiveInfo *info =
            new ArchiveInfo{pSelectedArchive, pSelectedArchive->Path,
                            pSelectedArchive->ImageVersion};
        CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)&IMGArchive::Save,
                     info, NULL, NULL);
  ...
  if (ImGui::BeginMenu("Edit")) {
    if (ImGui::MenuItem("Import", NULL, false, pSelectedArchive)) {
      ...
      ArchiveInfo *info =
          new ArchiveInfo{pSelectedArchive, fileNames, eImgVer::Unknown, false};
      CreateThread(NULL, NULL,
                   (LPTHREAD_START_ROUTINE)&IMGArchive::ImportEntries, info,
                   NULL, NULL);
    }
    if (ImGui::MenuItem("Import & replace", NULL, false, pSelectedArchive)) {
      std::string fileNames = WinDialogs::ImportFiles();
      ArchiveInfo *info =
          new ArchiveInfo{pSelectedArchive, fileNames, eImgVer::Unknown, true};
      CreateThread(NULL, NULL,
                   (LPTHREAD_START_ROUTINE)&IMGArchive::ImportEntries, info,
                   NULL, NULL);
    }
    if (ImGui::MenuItem("Export all", NULL, false,
                        pSelectedArchive &&
                            !pSelectedArchive->ProgressBar.bInUse)) {
      std::string path = WinDialogs::SaveFolder();
      ArchiveInfo *info = new ArchiveInfo{pSelectedArchive, path};
      CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)&IMGArchive::ExportAll,
                   info, NULL, NULL);
    }
    if (ImGui::MenuItem("Export selected", NULL, false,
                        pSelectedArchive &&
                            !pSelectedArchive->ProgressBar.bInUse)) {
      std::string path = WinDialogs::SaveFolder();
      ArchiveInfo *info = new ArchiveInfo{pSelectedArchive, path};
      CreateThread(NULL, NULL,
                   (LPTHREAD_START_ROUTINE)&IMGArchive::ExportSelected, info,
                   NULL, NULL);
    }
    ImGui::EndMenu();
  }
  ...
  if (ImGui::Begin("##Context", NULL, flags)) {
    if (ImGui::MenuItem("Delete")) {
      pSelectedArchive->EntryList.erase(
          std::remove_if(pSelectedArchive->EntryList.begin(),
                         pSelectedArchive->EntryList.end(),
                         [](EntryInfo const &obj) {
                           return !strcmp(obj.FileName,
                                          pContextEntry->FileName);
                         }),
          pSelectedArchive->EntryList.end());
  ...
  if (ImGui::BeginTabBar("Archives", tabFlags)) {
    for (IMGArchive &archive : ArchiveList) {
      if (ImGui::BeginTabItem(archive.FileName.c_str(), &archive.bOpen)) {
        ...
        // Search bar
        ImGui::SetNextItemWidth(ImGui::GetColumnWidth() - style.ItemSpacing.x -
                                style.WindowPadding.x);
        std::string hint = std::format("Search  {}", archive.Path);
        if (Widget::Filter("##Search", Filter, hint.c_str())) {
          pSelectedArchive->UpdateSelectList(Filter.InputBuf);
        }
        if (ImGui::BeginTable("ListedItems", 4,
          ...
          ImGuiListClipper clipper(
              static_cast<int>(pSelectedArchive->SelectedList.size()), height);
          while (clipper.Step()) {
            for (int i = clipper.DisplayStart; i < clipper.DisplayEnd; ++i) {
              ...
              if (Filter.PassFilter(pEntry->FileName)) {
                ...
                if (pEntry->bRename) {
                  ...
                  ImGui::InputText("##Rename", pEntry->FileName,
                                   sizeof(pEntry->FileName));
                  ...
                  if (ImGui::Selectable(pEntry->FileName, pEntry->bSelected)) {
                    ...
                    for (EntryInfo &e : archive.EntryList) {
                      ...
                      if (ImGui::IsKeyDown(VK_LSHIFT) ||
                          ImGui::IsKeyDown(VK_RSHIFT)) {
                        if (prevSelected) {
                          e.bSelected = true;
                        }
                        ...
                        if (!(ImGui::IsKeyDown(VK_LCONTROL) ||
                              ImGui::IsKeyDown(VK_RCONTROL))) {
                          e.bSelected = false;
                        }
                      }
                      e.bRename = false;
        ...
        if (ImGui::Button("Import", sz)) {
          std::string fileNames = WinDialogs::ImportFiles();
          ArchiveInfo *info = new ArchiveInfo{pSelectedArchive, fileNames,
                                              eImgVer::Unknown, false};
          CreateThread(NULL, NULL,
                       (LPTHREAD_START_ROUTINE)&IMGArchive::ImportEntries, info,
                       NULL, NULL);
        }
        ...
        if (ImGui::Button("Export all", sz) &&
          ...
          ArchiveInfo *info = new ArchiveInfo{pSelectedArchive, path};
          CreateThread(NULL, NULL,
                       (LPTHREAD_START_ROUTINE)&IMGArchive::ExportAll, info,
                       NULL, NULL);
        }
        ...
        if (ImGui::Button("Delete selection", sz)) {
          pSelectedArchive->EntryList.erase(
              std::remove_if(
                  ...
                  pSelectedArchive->EntryList.end(),
                  [](EntryInfo const &obj) { return obj.bSelected; }),
              pSelectedArchive->EntryList.end());
          pSelectedArchive->UpdateSelectList(Filter.InputBuf);
        }
        ImGui::SameLine();
        if (ImGui::Button("Dump list", sz)) {
          FILE *fp = fopen(std::format("C:/Users/Public/Desktop/{}.txt",
                                       pSelectedArchive->FileName)
                               .c_str(),
                           "w");
          if (fp) {
        ...
        if (ImGui::BeginTable(
                "Log", 1, ImGuiTableFlags_ScrollY | ImGuiTableFlags_Borders)) {
          // Freeze the header row
          ImGui::TableSetupScrollFreeze(0, 1);
      ...
      if (!archive.bOpen) {
        ArchiveList.erase(
            std::remove_if(ArchiveList.begin(), ArchiveList.end(),

                           [&archive](IMGArchive const &obj) {
                             return obj.bCreateNew
                                        ? obj.FileName == archive.FileName
                                        : obj.Path == archive.Path;
                           }),
            ArchiveList.end());
      }
    }
...
void Editor::Run() {
  ...
  Ui::eTheme theme = static_cast<Ui::eTheme>(Config.GetLongValue(
      "MAIN", "Theme", static_cast<long>(Ui::eTheme::SystemDefault)));
...
int WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine,
  ...
  Editor::AddArchiveEntry(exists ? IMGArchive(&lpCmdLine[1])
                                 : IMGArchive("Untitled", true));
  Updater::CheckUpdate();
  CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)&Updater::Process, NULL,
               NULL, NULL);
  Editor::Run();
  return 0;
}