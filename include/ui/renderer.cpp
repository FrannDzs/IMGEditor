...
bool Ui::Renderer::IsLightTheme() {
  ...
  // The value is expected to be a REG_DWORD, which is a signed 32-bit
  // little-endian
...
LRESULT WINAPI Ui::Renderer::WndProc(HWND hWnd, UINT msg, WPARAM wParam,
                                     LPARAM lParam) {
...
void Ui::Renderer::ProcessThemes() {
  ...
  if (isLightMode != prevIsLightMode || prevTheme != Theme) {
    if ((isLightMode && Theme == eTheme::SystemDefault) ||
        Theme == eTheme::Light) {
      useDarkMode = FALSE;
...
void Ui::Renderer::DrawLayer(Specification &Spec) {
  ...
  if (Spec.LayerFunc) {
    ...
    float height = static_cast<float>(rect.bottom - rect.top - start -
                                      ImGui::GetStyle().WindowPadding.y * 4.5);
  ...
  pd3dDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, clear_col_dx,
                    1.0f, 0);
  ...
  if (result == D3DERR_DEVICELOST &&
      pd3dDevice->TestCooperativeLevel() == D3DERR_DEVICENOTRESET) {
    ResetDevice();
  }
}
...
void Ui::Renderer::Init(const Specification &Spec) {
  ...
  RegisterClassEx(&wc);
  hwnd = CreateWindow(wc.lpszClassName, Spec.Name.c_str(), WS_OVERLAPPEDWINDOW,
                      Spec.Pos.x, Spec.Pos.y, Spec.Size.x, Spec.Size.y, NULL,
  ...
  io.FontDefault = io.Fonts->AddFontFromFileTTF(
      "C:\\Windows\\Fonts\\segoeui.ttf", 17.0f * h / 768);
  io.Fonts->Build();
...
void Ui::Renderer::SetThemeMode(eTheme themeMode) { Theme = themeMode; }

Ui::eTheme Ui::Renderer::GetThemeMode() { return Theme; }
