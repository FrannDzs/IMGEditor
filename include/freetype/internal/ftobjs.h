 ...
 */
#define FT_HYPOT(x, y)                                                         \
  (x = FT_ABS(x), y = FT_ABS(y), x > y ? x + (3 * y >> 3) : y + (3 * x >> 3))
...
#define ft_isxdigit(x)                                                         \
  (((unsigned)(x) - '0') < 10U || ((unsigned)(x) - 'a') < 6U ||                \
   ((unsigned)(x) - 'A') < 6U)
...
typedef FT_UInt (*FT_CMap_CharVarIndexFunc)(FT_CMap cmap, FT_CMap unicode_cmap,
                                            FT_UInt32 char_code,
                                            FT_UInt32 variant_selector);

typedef FT_Int (*FT_CMap_CharVarIsDefaultFunc)(FT_CMap cmap,
                                               FT_UInt32 char_code,
                                               FT_UInt32 variant_selector);

typedef FT_UInt32 *(*FT_CMap_VariantListFunc)(FT_CMap cmap, FT_Memory mem);

typedef FT_UInt32 *(*FT_CMap_CharVariantListFunc)(FT_CMap cmap, FT_Memory mem,
                                                  FT_UInt32 char_code);

typedef FT_UInt32 *(*FT_CMap_VariantCharListFunc)(FT_CMap cmap, FT_Memory mem,
                                                  FT_UInt32 variant_selector);
...
#define FT_DEFINE_CMAP_CLASS(                                                  \
    class_, size_, init_, done_, char_index_, char_next_, char_var_index_,     \
    char_var_default_, variant_list_, charvariant_list_, variantchar_list_)    \
  FT_CALLBACK_TABLE_DEF                                                        \
  const FT_CMap_ClassRec class_ = {size_,                                      \
                                   init_,                                      \
                                   done_,                                      \
                                   char_index_,                                \
                                   char_next_,                                 \
...
FT_BASE(FT_Error)
FT_CMap_New(FT_CMap_Class clazz, FT_Pointer init_data, FT_CharMap charmap,
            FT_CMap *acmap);
...
typedef void (*FT_Bitmap_LcdFilterFunc)(FT_Bitmap *bitmap, FT_Byte *weights);

/* This is the default LCD filter, an in-place, 5-tap FIR filter. */
FT_BASE(void)
ft_lcd_filter_fir(FT_Bitmap *bitmap, FT_LcdFiveTapFilter weights);
...
#define FT_MODULE_IS_DRIVER(x)                                                 \
  (FT_MODULE_CLASS(x)->module_flags & FT_MODULE_FONT_DRIVER)

#define FT_MODULE_IS_RENDERER(x)                                               \
  (FT_MODULE_CLASS(x)->module_flags & FT_MODULE_RENDERER)

#define FT_MODULE_IS_HINTER(x)                                                 \
  (FT_MODULE_CLASS(x)->module_flags & FT_MODULE_HINTER)

#define FT_MODULE_IS_STYLER(x)                                                 \
  (FT_MODULE_CLASS(x)->module_flags & FT_MODULE_STYLER)

#define FT_DRIVER_IS_SCALABLE(x)                                               \
  (FT_MODULE_CLASS(x)->module_flags & FT_MODULE_DRIVER_SCALABLE)

#define FT_DRIVER_USES_OUTLINES(x)                                             \
  !(FT_MODULE_CLASS(x)->module_flags & FT_MODULE_DRIVER_NO_OUTLINES)

#define FT_DRIVER_HAS_HINTER(x)                                                \
  (FT_MODULE_CLASS(x)->module_flags & FT_MODULE_DRIVER_HAS_HINTER)

#define FT_DRIVER_HINTS_LIGHTLY(x)                                             \
  (FT_MODULE_CLASS(x)->module_flags & FT_MODULE_DRIVER_HINTS_LIGHTLY)
...
FT_BASE(FT_Error)
ft_property_string_set(FT_Library library, const FT_String *module_name,
                       const FT_String *property_name, FT_String *value);
#endif
...
FT_BASE(FT_Error)
FT_Match_Size(FT_Face face, FT_Size_Request req, FT_Bool ignore_width,
              FT_ULong *size_index);
...
FT_BASE(FT_Bool)
ft_glyphslot_preset_bitmap(FT_GlyphSlot slot, FT_Render_Mode mode,
...
FT_BASE(FT_Renderer)
FT_Lookup_Renderer(FT_Library library, FT_Glyph_Format format,
                   FT_ListNode *node);

FT_BASE(FT_Error)
FT_Render_Glyph_Internal(FT_Library library, FT_GlyphSlot slot,
                         FT_Render_Mode render_mode);

typedef const char *(*FT_Face_GetPostscriptNameFunc)(FT_Face face);

typedef FT_Error (*FT_Face_GetGlyphNameFunc)(FT_Face face, FT_UInt glyph_index,
                                             FT_Pointer buffer,
                                             FT_UInt buffer_max);

typedef FT_UInt (*FT_Face_GetGlyphNameIndexFunc)(FT_Face face,
...
#define FT_DEFINE_GLYPH(class_, size_, format_, init_, done_, copy_,           \
  ...
  FT_CALLBACK_TABLE_DEF                                                        \
  const FT_Glyph_Class class_ = {size_, format_,    init_, done_,              \
                                 copy_, transform_, bbox_, prepare_};
...
#define FT_DEFINE_RENDERER(class_, flags_, size_, name_, version_, requires_,  \
