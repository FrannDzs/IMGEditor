...
#define FT_DEBUG_INNER(exp)                                                    \
  (_ft_debug_file = __FILE__, _ft_debug_lineno = __LINE__, (exp))

#define FT_ASSIGNP_INNER(p, exp)                                               \
  (_ft_debug_file = __FILE__, _ft_debug_lineno = __LINE__, FT_ASSIGNP(p, exp))
...
FT_BASE(FT_Pointer)
ft_mem_realloc(FT_Memory memory, FT_Long item_size, FT_Long cur_count,
...
#define FT_MEM_REALLOC(ptr, cursz, newsz)                                      \
  FT_ASSIGNP_INNER(ptr, ft_mem_realloc(memory, 1, (FT_Long)(cursz),            \
                                       (FT_Long)(newsz), (ptr), &error))
...
#define FT_MEM_QREALLOC(ptr, cursz, newsz)                                     \
  FT_ASSIGNP_INNER(ptr, ft_mem_qrealloc(memory, 1, (FT_Long)(cursz),           \
                                        (FT_Long)(newsz), (ptr), &error))

#define FT_MEM_ALLOC_MULT(ptr, count, item_size)                               \
  FT_ASSIGNP_INNER(ptr, ft_mem_realloc(memory, (FT_Long)(item_size), 0,        \
                                       (FT_Long)(count), NULL, &error))

#define FT_MEM_REALLOC_MULT(ptr, oldcnt, newcnt, itmsz)                        \
  FT_ASSIGNP_INNER(ptr,                                                        \
                   ft_mem_realloc(memory, (FT_Long)(itmsz), (FT_Long)(oldcnt), \
                                  (FT_Long)(newcnt), (ptr), &error))
...
#define FT_MEM_QREALLOC_MULT(ptr, oldcnt, newcnt, itmsz)                       \
  FT_ASSIGNP_INNER(ptr, ft_mem_qrealloc(memory, (FT_Long)(itmsz),              \
                                        (FT_Long)(oldcnt), (FT_Long)(newcnt),  \
                                        (ptr), &error))
...
#define FT_ARRAY_ZERO(dest, count)                                             \
  FT_MEM_ZERO(dest, (FT_Offset)(count) * sizeof(*(dest)))

#define FT_ARRAY_COPY(dest, source, count)                                     \
  FT_MEM_COPY(dest, source, (FT_Offset)(count) * sizeof(*(dest)))

#define FT_ARRAY_MOVE(dest, source, count)                                     \
  FT_MEM_MOVE(dest, source, (FT_Offset)(count) * sizeof(*(dest)))
...
#define FT_MEM_NEW_ARRAY(ptr, count)                                           \
  FT_ASSIGNP_INNER(ptr, ft_mem_realloc(memory, sizeof(*(ptr)), 0,              \
                                       (FT_Long)(count), NULL, &error))

#define FT_MEM_RENEW_ARRAY(ptr, cursz, newsz)                                  \
  FT_ASSIGNP_INNER(ptr,                                                        \
                   ft_mem_realloc(memory, sizeof(*(ptr)), (FT_Long)(cursz),    \
                                  (FT_Long)(newsz), (ptr), &error))

#define FT_MEM_QNEW_ARRAY(ptr, count)                                          \
  FT_ASSIGNP_INNER(ptr, ft_mem_qrealloc(memory, sizeof(*(ptr)), 0,             \
                                        (FT_Long)(count), NULL, &error))

#define FT_MEM_QRENEW_ARRAY(ptr, cursz, newsz)                                 \
  FT_ASSIGNP_INNER(ptr,                                                        \
                   ft_mem_qrealloc(memory, sizeof(*(ptr)), (FT_Long)(cursz),   \
                                   (FT_Long)(newsz), (ptr), &error))
...
FT_BASE(FT_Pointer)
ft_mem_dup(FT_Memory memory, const void *address, FT_ULong size,
           FT_Error *p_error);
