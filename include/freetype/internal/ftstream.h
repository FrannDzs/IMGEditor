...
#define FT_PEEK_LONG(p)                                                        \
  FT_INT32(FT_BYTE_U32(p, 0, 24) | FT_BYTE_U32(p, 1, 16) |                     \
           FT_BYTE_U32(p, 2, 8) | FT_BYTE_U32(p, 3, 0))

#define FT_PEEK_ULONG(p)                                                       \
  FT_UINT32(FT_BYTE_U32(p, 0, 24) | FT_BYTE_U32(p, 1, 16) |                    \
            FT_BYTE_U32(p, 2, 8) | FT_BYTE_U32(p, 3, 0))
...
#define FT_PEEK_SHORT_LE(p)                                                    \
  FT_INT16(FT_BYTE_U16(p, 1, 8) | FT_BYTE_U16(p, 0, 0))

#define FT_PEEK_USHORT_LE(p)                                                   \
  FT_UINT16(FT_BYTE_U16(p, 1, 8) | FT_BYTE_U16(p, 0, 0))

#define FT_PEEK_LONG_LE(p)                                                     \
  FT_INT32(FT_BYTE_U32(p, 3, 24) | FT_BYTE_U32(p, 2, 16) |                     \
           FT_BYTE_U32(p, 1, 8) | FT_BYTE_U32(p, 0, 0))

#define FT_PEEK_ULONG_LE(p)                                                    \
  FT_UINT32(FT_BYTE_U32(p, 3, 24) | FT_BYTE_U32(p, 2, 16) |                    \
            FT_BYTE_U32(p, 1, 8) | FT_BYTE_U32(p, 0, 0))

#define FT_PEEK_OFF3_LE(p)                                                     \
  (FT_INT32(FT_BYTE_U32(p, 2, 24) | FT_BYTE_U32(p, 1, 16) |                    \
            FT_BYTE_U32(p, 0, 8)) >>                                           \
   8)
...
#define FT_NEXT_SHORT(buffer) (buffer += 2, FT_PEEK_SHORT(buffer - 2))

#define FT_NEXT_USHORT(buffer) (buffer += 2, FT_PEEK_USHORT(buffer - 2))

#define FT_NEXT_OFF3(buffer) (buffer += 3, FT_PEEK_OFF3(buffer - 3))

#define FT_NEXT_UOFF3(buffer) (buffer += 3, FT_PEEK_UOFF3(buffer - 3))

#define FT_NEXT_LONG(buffer) (buffer += 4, FT_PEEK_LONG(buffer - 4))

#define FT_NEXT_ULONG(buffer) (buffer += 4, FT_PEEK_ULONG(buffer - 4))

#define FT_NEXT_SHORT_LE(buffer) (buffer += 2, FT_PEEK_SHORT_LE(buffer - 2))

#define FT_NEXT_USHORT_LE(buffer) (buffer += 2, FT_PEEK_USHORT_LE(buffer - 2))

#define FT_NEXT_OFF3_LE(buffer) (buffer += 3, FT_PEEK_OFF3_LE(buffer - 3))

#define FT_NEXT_UOFF3_LE(buffer) (buffer += 3, FT_PEEK_UOFF3_LE(buffer - 3))

#define FT_NEXT_LONG_LE(buffer) (buffer += 4, FT_PEEK_LONG_LE(buffer - 4))

#define FT_NEXT_ULONG_LE(buffer) (buffer += 4, FT_PEEK_ULONG_LE(buffer - 4))
...
#define FT_READ_MACRO(func, type, var)                                         \
  (var = (type)func(stream, &error), error != FT_Err_Ok)
...
#define FT_READ_SHORT_LE(var)                                                  \
  FT_READ_MACRO(FT_Stream_ReadUShortLE, FT_Int16, var)
#define FT_READ_USHORT_LE(var)                                                 \
  FT_READ_MACRO(FT_Stream_ReadUShortLE, FT_UInt16, var)
#define FT_READ_LONG_LE(var) FT_READ_MACRO(FT_Stream_ReadULongLE, FT_Int32, var)
#define FT_READ_ULONG_LE(var)                                                  \
  FT_READ_MACRO(FT_Stream_ReadULongLE, FT_UInt32, var)
...
#define FT_STREAM_SEEK(position)                                               \
  FT_SET_ERROR(FT_Stream_Seek(stream, (FT_ULong)(position)))

#define FT_STREAM_SKIP(distance)                                               \
  FT_SET_ERROR(FT_Stream_Skip(stream, (FT_Long)(distance)))

#define FT_STREAM_READ(buffer, count)                                          \
  FT_SET_ERROR(FT_Stream_Read(stream, (FT_Byte *)(buffer), (FT_ULong)(count)))

#define FT_STREAM_READ_AT(position, buffer, count)                             \
  FT_SET_ERROR(FT_Stream_ReadAt(stream, (FT_ULong)(position),                  \
...
#define FT_FRAME_ENTER(size)                                                   \
  FT_SET_ERROR(FT_DEBUG_INNER(FT_Stream_EnterFrame(stream, (FT_ULong)(size))))

#define FT_FRAME_EXIT() FT_DEBUG_INNER(FT_Stream_ExitFrame(stream))

#define FT_FRAME_EXTRACT(size, bytes)                                          \
  FT_SET_ERROR(FT_DEBUG_INNER(                                                 \
      FT_Stream_ExtractFrame(stream, (FT_ULong)(size), (FT_Byte **)&(bytes))))
