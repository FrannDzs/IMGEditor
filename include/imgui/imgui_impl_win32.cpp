// dear imgui: Platform Backend for Windows (standard windows API for 32 and 64
...
//  dear imgui) [X] Platform: Keyboard support. Since 1.87 we are using the
//  io.AddKeyEvent() function. Pass ImGuiKey values to all key functions e.g.
//  ImGui::IsKeyPressed(ImGuiKey_Space). [Legacy VK_* values will also be
...
//  mods next and before a key event (not in NewFrame) to fix input queue with
//  very low framerates. 2022-01-12: Inputs: Update mouse inputs using
...
//  mask instead of using ImGui::IsAnyMouseDown() which will be obsoleted.
...
//  area when dragging. 2016-11-12: Inputs: Only call Win32 ::SetCursor(NULL)
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg,
  ...
  case WM_MOUSEMOVE:
    ...
    io.AddMousePosEvent((float)GET_X_LPARAM(lParam),
