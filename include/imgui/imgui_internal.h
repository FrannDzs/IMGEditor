...
// #define ImFloorStd(X)     floorf(X)           // We use our own, see
// ImFloor() and ImFloorSigned()
...
enum ImGuiItemFlags_ {
  ...
  ImGuiItemFlags_NoTabStop = 1 << 0, // false     // Disable keyboard tabbing
                                     // (FIXME: should merge with _NoNav)
  ...
  ImGuiItemFlags_NoNav =
      1 << 3, // false     // Disable keyboard/gamepad directional navigation
              // (FIXME: should merge with _NoTabStop)
  ...
  ImGuiItemFlags_Inputable =
      1 << 8 // false     // [WIP] Auto-activate input mode when tab focused.
...
enum ImGuiItemStatusFlags_ {
  ...
  ImGuiItemStatusFlags_FocusedByTabbing =
      1 << 8 // Set when the Focusable item just got focused by Tabbing (FIXME:
             // to be removed soon)
...
enum ImGuiSelectableFlagsPrivate_ {
  ...
  ImGuiSelectableFlags_DrawHoveredWhenHeld =
      1 << 25, // Always show active when held, even is not hovered. This
  ...
  ImGuiSelectableFlags_NoPadWithHalfSpacing =
      1 << 27 // Disable padding each side with ItemSpacing * 0.5f
};
...
struct IMGUI_API ImGuiInputTextState {
  ...
  ImVector<char> InitialTextA; // backup of end-user buffer at the time of focus
                               // (in UTF-8, unaltered)
...
struct ImGuiPopupData {
  ...
  ImGuiWindow *SourceWindow; // Set on OpenPopup() copy of NavWindow at the time
                             // of opening the popup
...
struct ImGuiNextItemData {
  ...
  ImGuiID FocusScopeId; // Set by SetNextItemMultiSelectData() (!= 0 signify
                        // value has been set, so it's an alternate version of
                        // HasSelectionData, we don't use Flags for this because
                        // they are cleared too early. This is mostly used for
                        // debugging)
  ImGuiCond OpenCond;
...
struct ImGuiLastItemData {
  ...
  ImRect DisplayRect; // Display rectangle (only if
...
enum ImGuiActivateFlags_ {
  ...
  ImGuiActivateFlags_PreferTweak =
      1 << 1, // Favor activation for tweaking with arrows or gamepad (e.g. for
...
enum ImGuiNavDirSourceFlags_ {
  ...
  ImGuiNavDirSourceFlags_RawKeyboard =
      1 << 0, // Raw keyboard (not pulled from nav), facilitate use of some
              // functions before we can unify nav and keys
...
enum ImGuiNavMoveFlags_ {
  ...
  ImGuiNavMoveFlags_Tabbing = 1 << 10, // == Focus + Activate if item is
...
struct ImGuiViewportP : public ImGuiViewport {
  int DrawListsLastFrame[2]; // Last frame number the background (0) and
                             // foreground (1) draw lists were used
  ImDrawList *DrawLists[2];  // Convenience background (0) and foreground (1)
...
struct ImGuiMetricsConfig {
  ...
  ImGuiMetricsConfig() {
    ShowDebugLog = ShowStackTool = ShowWindowsRects = ShowWindowsBeginOrder =
...
struct ImGuiContext {
  ...
  ImVector<ImGuiInputEvent>
      InputEventsQueue; // Input events which will be tricked/written into IO
                        // structure.
  ImVector<ImGuiInputEvent>
  ...
  ImGuiID
      DebugHookIdInfo; // Will call core hooks: DebugHookIdInfo() from GetID
                       // functions, used by Stack Tool [next HoveredId/ActiveId
                       // to not pull in an extra cache-line]
  ImGuiID HoveredId;   // Hovered widget, filled during the frame
  ImGuiID HoveredIdPreviousFrame;
  bool HoveredIdAllowOverlap;
  bool HoveredIdUsingMouseWheel; // Hovered widget will use mouse wheel. Blocks
                                 // scrolling the underlying window.
  ...
  // Input Ownership
  bool ActiveIdUsingMouseWheel;  // Active widget will want to read mouse wheel.
                                 // Blocks scrolling the underlying window.
  ...
  ImU32 ActiveIdUsingNavInputMask; // Active widget will want to read those nav
                                   // inputs.
  ImBitArrayForNamedKeys
      ActiveIdUsingKeyInputMask; // Active widget will want to read those key
                                 // inputs. When we grow the ImGuiKey enum we'll
                                 // need to either to order the enum to make
                                 // useful keys come first, either redesign this
  ...
  ImGuiID NavFocusScopeId; // Identify a selection scope (selection code often
  ...
  ImGuiID NavActivatePressedId; // ~~ IsNavInputPressed(ImGuiNavInput_Activate)
                                // ? NavId : 0
  ImGuiID NavActivateInputId;   // ~~ IsNavInputPressed(ImGuiNavInput_Input) ?
                              // NavId : 0; ImGuiActivateFlags_PreferInput will
  ...
  ImGuiID NavInitResultId; // Init request result (first item of the window, or
                           // one for which SetItemDefaultFocus() was called)
  ImRect NavInitResultRectRel; // Init request result rectangle (relative to
                               // parent window)
  ...
  float ColorEditLastHue; // Backup of last Hue associated to LastColor, so we
                          // can restore Hue in lossy RGB<>HSV round trips
  float ColorEditLastSat; // Backup of last Saturation associated to LastColor,
  ...
  float TooltipSlowDelay; // Time before slow tooltips appears (FIXME: This is
                          // temporary until we merge in tooltip timer+priority
                          // work)
  ImVector<char>
  ...
  ImGuiContext(ImFontAtlas *shared_font_atlas) {
    ...
    NavId = NavFocusScopeId = NavActivateId = NavActivateDownId =
        NavActivatePressedId = NavActivateInputId = 0;
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  ImVec2
      CursorStartPosLossyness; // Record the loss of precision of CursorStartPos
                               ...
                               // used by clipper to compensentate and fix the
...
#define IMGUI_TABLE_MAX_COLUMNS                                                \
  64 // sizeof(ImU64) * 8. This is solely because we frequently encode columns
     // set in a ImU64.
...
struct ImGuiTableColumn {
  ...
  ImU8 SortDirectionsAvailMask : 4;  // Mask of available sort directions (1-bit
                                     // each)
  ImU8 SortDirectionsAvailList; // Ordered of available sort directions (2-bits
                                // each)
...
// do not need to be preserved aside from debug needs, does that needs they
// could be moved to ImGuiTableTempData ?)
struct ImGuiTableInstanceData {
  float LastOuterHeight;    // Outer height from last frame // FIXME:
                            // multi-instance issue (#3955)
  float LastFirstRowHeight; // Height of first row from last frame // FIXME:
                            // possible multi-instance issue?

  ImGuiTableInstanceData() { LastOuterHeight = LastFirstRowHeight = 0.0f; }
};

// FIXME-TABLE: more transient data could be stored in a per-stacked table
...
struct IMGUI_API ImGuiTable {
  ...
  ImU64 EnabledMaskByIndex; // Column Index -> IsEnabled map (== not hidden by
                            // user/api) in a format adequate for iterating
                            // column without touching cold data
  ImU64 VisibleMaskByIndex; // Column Index -> IsVisibleX|IsVisibleY map (== not
                            // hidden by user/api && not hidden by
                            // scrolling/cliprect)
...
// If you have old/custom copy-and-pasted widgets that used
...
//  (GetItemStatusFlags() & ImGuiItemStatusFlags_Focused) != 0' (New)
//  IMGUI_VERSION_NUM >= 18413: using 'ItemAdd(..., ImGuiItemFlags_Inputable)'
//  and 'bool tab_focused = (GetItemStatusFlags() &
//  ImGuiItemStatusFlags_FocusedTabbing) != 0 || g.NavActivateInputId == id'
//  (WIP)
...
IMGUI_API void BeginTooltipEx(ImGuiTooltipFlags tooltip_flags,
